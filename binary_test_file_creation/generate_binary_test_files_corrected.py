...
def create_frequency_mask(n_freq_points, sampling_rate=16000, f_min=300.0, f_max=7900.0):
...
def process_test_files():
    """Process test files for cars 1-4 and generate individual .bin files with corrected frequency range"""
    
    base_path = "/Users/jaymain/MLPerf Tiny C++ MBED/FFT_ave_800_no_log"
    output_dir = "/Users/jaymain/MLPerf Tiny C++ MBED/test_samples_corrected"
    ...
    # Create frequency mask with corrected parameters
    freq_mask = create_frequency_mask(n_freq_points, f_min=300.0, f_max=7900.0)
    final_feature_count = np.sum(freq_mask)
    
    print(f"=== Corrected Binary File Generation ===")
    print(f"Original FFT features: {n_freq_points}")
    print(f"Frequency range: 300.0 Hz to 7900.0 Hz")
    ...
    for car_id in car_ids:
        for pattern, label in file_patterns:
            ...
            for sample_idx in range(masked_data.shape[0]):
                sample_data = masked_data[sample_idx]  # Shape: (103,)
    ...
    with open(summary_path, 'w') as f:
        f.write("Binary Test Files for MLPerf Tiny Anomaly Detection (CORRECTED)\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"Frequency range: 300.0-7900.0 Hz (after masking)\n")
        f.write(f"This corrects the previous 1200-7900 Hz range\n\n")
...
if __name__ == "__main__":
    ...
    print(f"✅ C++ model_config.h updated to use {final_feature_count} features")
    print(f"✅ Ridge classifier weights are correct (103 features + 1 bias = 104 total)")
    print(f"✅ System is ready for MLPerf Tiny benchmarking")